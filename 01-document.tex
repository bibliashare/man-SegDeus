...
\section{Introdução}
    ...
    coisas\/}''\footnote{\FRtxt{Qui est à l'extrémité, extrème, dernier ... \textbf{II} (avec idée de  temps):  ...  dernier}.},
    juntamente com \GRtxt{-λογία}: palavras~\cite{1997-ManiatoglouMPF-Porto} (estudo), a saber: o estudo das últimas coisas, com
    ...
    ``meio-termo'' --- significando que para  uma  dada  escatologia  $\epsilon_i,  i  \in  \{1,  2,  3,  \ldots\}$,  ou  ela  é
    \bQuote{segundo Deus} ou ela é  ``com  erro,''  pois  se  (i)~$\epsilon_i$  incorrer  em  erro  (contiver  erro),  não  será
    \bQuote{segundo Deus}, pois está escrito:
    ...
    \noindent ou se (ii)~$\epsilon_i$ não contiver erro,  terá  sido  feita  \bLineQuote{segundo  Deus,  em  justiça  e  retidão
    procedentes da verdade}{Ef~2.24}{ARA} e será, portanto, \bQuote{segundo Deus}.

    As Definições~\ref{def.esc.segDeus} e~\ref{def.esc.comerro} dadas são úteis  na  \emph{classificação}  de  escatologias  que
    \textbf{devem ser abandonadas} por membros do corpo de Cristo (à luz de Ef~4.7,11--14, já citado), e aquela, \bQuote{segundo
    Deus}, que \textbf{deve ser guardada} e também \emph{crida, ensinada e proclamada}.

    Entretanto, as Definições~\ref{def.esc.segDeus}  e~\ref{def.esc.comerro}  dadas,  não  são,  necessariamente,  de  fácil  ou
    consensual aplicação, especialmente em um cenário --- o atual --- no qual proliferam, não apenas diferentes visões de mundo,
    mas igualmente, erros grosseiros nas diferentes abordagens escatológicas, além de evidência de muito desapreço por  exatidão
    e verdade.

    De  um  ponto  de  vista  prático,  se  um  número  razoável  de  \emph{princípios}  for  identificado,   tais   que   sejam
    (a)~\emph{indispensáveis} a estudos proféticos \bQuote{segundo Deus}, e que também sejam (b)~de fácil verificação  quanto  à
    sua violação; ter-se-á uma metodologia de verificação de escatologias com erro  (Definição~\ref{def.esc.comerro}),  uma  vez
    que, de acordo com o caso~(i) anteriormente enunciado,  se  uma  escatologia  $\epsilon_i$  violar  \emph{qualquer  um}  dos
    princípios reunidos, os quais são \emph{indispensáveis} a estudos proféticos \bQuote{segundo Deus}, qualifica-se e  expõe-se
    o erro incorrido pela escatologia em questão!


    a identificação princípios

    a  existência  de  \emph{princípios  mais  facilmente   verificáveis},   e   que   sejam
    \emph{indispensáveis}  a  estudos  proféticos  \bQuote{segundo  Deus},  são  mais  imediatamente  aplicáveis,  e,  portanto,
    desejáveis, do que as Definições~\ref{def.esc.segDeus} e~\ref{def.esc.comerro} dadas.


    %---------------------------------------------------------------------------------------------------------------------------
    \subsection{Objetivos Gerais}

    Este estudo objetiva \emph{identificar} e \emph{enunciar}, em acordo com as Escrituras, \emph{princípios bíblicos} tidos por
    \emph{indispensáveis} para que uma escatologia $\epsilon_i$, que fielmente os siga, \emph{possa ser} \bQuote{segundo  Deus},
    isto é, princípios bíblicos \emph{necessários}, porém  \emph{não  suficientes}  a  que  a  escatologia  que  os  siga  tenha
    possibilidade de ser \bQuote{segundo Deus}.
    ...
    %---------------------------------------------------------------------------------------------------------------------------
    \subsection{Princípios Bíblicos}

    Desta forma, este estudo procura identificar \emph{princípios bíblicos}, deduzidos exclusivamente à partir  das  Escrituras,
    que sejam \emph{indispensáveis} a estudos bíblicos proféticos \bQuote{segundo Deus}, porém  \emph{não  suficientes}.  Assim,
    estudos bíblicos  proféticos  \bQuote{segundo  Deus}  \textbf{não  violam  nenhum}  dos  princípios,  pois  são,  segundo  a
    Definição~\ref{def.esc.segDeus}, ``basead(os) unicamente em verdade e conduzid(os) em retidão''.

    Por outro lado, se um dado estudo escatológico ou profético demonstradamente violar \emph{qualquer um} dos  \emph{princípios
    bíblicos}, em tal estudo haverá engano e mentira, que se  propagarão  às  conclusões  do  estudo,  tornando  tal  estudo  em
    escatologia enganosa pela Definição~\ref{def.esc.comerro}.

    Assim, se um estdo \emph{não viola} qualquer princípio  das  Escrituras  a  ser  aqui  identificado,  tal  estudo  \emph{tem
    possibilidade} de ser \bQuote{segundo Deus}, mas não necessariamente a garantia. Já aqueles que  violam  qualquer  princípio
...
\section{Princípios Bíblicos Para Escatologia ``Segundo Deus''}
    ...
    mesmos para provar que um estudo é \bQuote{segundo Deus}.
    ...
    % !j 116 -i8
    \begin{DEF}[Princípio da Unicidade da Realidade]
    ...
    meio das \textbf{coisas que foram criadas}.}{Rm~1.20}{ARA}, de sorte que hoje sabemos que Deus está a evocar Seus  atributos
    de \bQuote{eterno poder, como também a sua própria divindade} ao declarar-se Autor de céus e terra, quando falou por meio do
    profeta Isaías.
    ...
    % !j 116 -i8
    \begin{DEF}[Princípio da Verdade das Profecias Divinas]
    ...
    % !j 116 -i8
    \begin{DEF}[Princípio das Profecias Como Promessas]
    ...
    % !j 116 -i8
    \begin{DEF}[Princípio da Verificabilidade Das Profecias Divinas]
    ...
    % !j 116 -i8
    \begin{DEF}[Princípio da Vigilância Divina à Sua Palavra]
        \label{def.pri.vigilância}
        Deus mantém constante vigilância à toda a sua Palavra, com o propósito de cumprí-la.
    \end{DEF}
    ...
    % !j 116 -i8
    \begin{DEF}[Princípio do Cumprimento Como Profetizado]
    ...
    % !j 116 -i8
    \begin{COR}[Cumprimento Como Profetizado]
        \label{def.cor.comoprof}
        Deus não dá profecia de forma alegórica, com sentido oculto e diferente do que está escrito.
    \end{COR}

    E ainda, de acordo com a Escritura: \bLineQuote{Na verdade, \textbf{Deus não procede maliciosamente}}{Jó~34.12}{ARA}, que
    juntamente com o Corolário~\ref{def.cor.comoprof}, deriva-se:

    % !j 116 -i8
    \begin{COR}[Genuína Intenção de Deus]
        \label{def.cor.genu}
...
\section{Conclusão}
    ...
    Crê-se também que  \emph{qualquer}  estudo  Bíblico  feito  \bQuote{segundo  Deus}  levará  à  \emph{unidade  da  fé},  pois
    \bLineQuote{o Espírito dá testemunho, porque o Espírito é a verdade. E três são as testemunhas:  o  Espírito,  a  água  e  o
    sangue, e os três concordam.}{1Jo~5.7,8}{PESH}.
